...
const char * const EZ25COIN_CONF_FILENAME = "ez25coin.conf";
const char * const EZ25COIN_PID_FILENAME = "ez25coind.pid";
...
public:
    ...
    static inline std::pair<bool,std::string> GetArg(const ArgsManager &am, const std::string& arg)
    {
        ...
        // argument value seen from the command line (so "ez25coind -foo=bar
...
{
    // Windows < Vista: C:\Documents and Settings\Username\Application Data\EZ25COIN
    // Windows >= Vista: C:\Users\Username\AppData\Roaming\EZ25COIN
    // Mac: ~/Library/Application Support/EZ25COIN
    // Unix: ~/.ez25coin
#ifdef WIN32
    // Windows
    return GetSpecialFolderPath(CSIDL_APPDATA) / "EZ25COIN";
#else
    fs::path pathRet;
...
#ifdef MAC_OSX
    // Mac
    return pathRet / "Library/Application Support/EZ25COIN";
#else
    // Unix
    return pathRet / ".ez25coin";
#endif
#endif
}
...
bool ArgsManager::ReadConfigFiles(std::string& error, bool ignore_invalid_keys)
{
    ...
    const std::string confPath = GetArg("-conf", EZ25COIN_CONF_FILENAME);
...
fs::path GetPidFile()
{
    return AbsPathForConfigVal(fs::path(gArgs.GetArg("-pid", EZ25COIN_PID_FILENAME)));
}
