...
###   ez25coincore.org and ez25coin.org and compares them.
...
WORKINGDIR="/tmp/ez25coin_verify_binaries"
TMPFILE="hashes.tmp"

SIGNATUREFILENAME="SHA256SUMS.asc"
RCSUBDIR="test"
HOST1="https://ez25coincore.org"
HOST2="https://ez25coin.org"
BASEDIR="/bin/"
VERSIONPREFIX="ez25coin-core-"
RCVERSIONSTRING="rc"
...
if [ -n "$1" ]; then
   #let's also check if the version number includes the prefix 'ez25coin-',
...
if [ $? -ne 0 ]; then
   echo "ez25coin.org failed to provide signature file, but ez25coincore.org did?"
   echo "wget output:"
   echo "$WGETOUT"|sed 's/^/\t/g'
...
if [ "$SIGFILEDIFFS" != "" ]; then
   echo "ez25coin.org and ez25coincore.org signature files were not equal?"
...
if [ $RET -ne 0 ]; then
   if [ $RET -eq 1 ]; then
      ...
      echo "gpg error. Do you have the EZ25COIN Core binary release signing key installed?"
   fi
