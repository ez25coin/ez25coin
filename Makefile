...
pkgdatadir = $(datadir)/ez25coin
pkgincludedir = $(includedir)/ez25coin
pkglibdir = $(libdir)/ez25coin
pkglibexecdir = $(libexecdir)/ez25coin
...
am__aclocal_m4_deps = $(top_srcdir)/build-aux/m4/ax_boost_base.m4 \
	...
	$(top_srcdir)/build-aux/m4/ez25coin_find_bdb48.m4 \
	$(top_srcdir)/build-aux/m4/ez25coin_qt.m4 \
	$(top_srcdir)/build-aux/m4/ez25coin_subdir_to_include.m4 \
...
CONFIG_HEADER = $(top_builddir)/src/config/ez25coin-config.h
CONFIG_CLEAN_FILES = libez25coinconsensus.pc share/setup.nsi \
...
CONFIG_CLEAN_VPATH_FILES = contrib/filter-lcov.py \
	test/functional/test_runner.py test/util/ez25coin-util-test.py \
...
am__DIST_COMMON = $(srcdir)/Makefile.in \
	$(srcdir)/libez25coinconsensus.pc.in \
	...
	$(top_srcdir)/src/config/ez25coin-config.h.in \
	$(top_srcdir)/test/config.ini.in \
	$(top_srcdir)/test/functional/test_runner.py \
	$(top_srcdir)/test/util/ez25coin-util-test.py \
...
ACLOCAL = ${SHELL} /tmp/EZ25COIN/build-aux/missing aclocal-1.15
AMTAR = $${TAR-tar}
AM_DEFAULT_VERBOSITY = 0
AR = /usr/bin/ar
ARFLAGS = cr
AUTOCONF = ${SHELL} /tmp/EZ25COIN/build-aux/missing autoconf
AUTOHEADER = ${SHELL} /tmp/EZ25COIN/build-aux/missing autoheader
AUTOMAKE = ${SHELL} /tmp/EZ25COIN/build-aux/missing automake-1.15
...
COPYRIGHT_HOLDERS_FINAL = The EZ25COIN Core developers
COPYRIGHT_HOLDERS_SUBSTITUTION = EZ25COIN Core
COPYRIGHT_YEAR = 2019
CPP = gcc -E
...
EZ25COIN_CLI_NAME = ez25coin-cli
EZ25COIN_DAEMON_NAME = ez25coind
EZ25COIN_GUI_NAME = ez25coin-qt
EZ25COIN_TX_NAME = ez25coin-tx
FGREP = /bin/grep -F
GCOV = /usr/bin/gcov
GENHTML = 
GENISOIMAGE = 
...
MAINT = 
MAKEINFO = ${SHELL} /tmp/EZ25COIN/build-aux/missing makeinfo
MAKENSIS = 
MANIFEST_TOOL = :
MINIUPNPC_CPPFLAGS = 
...
OTOOL64 = 
PACKAGE = ez25coin
PACKAGE_BUGREPORT = https://github.com/ez25coin/ez25coin/issues
PACKAGE_NAME = EZ25COIN Core
PACKAGE_STRING = EZ25COIN Core 1.0.0
PACKAGE_TARNAME = ez25coin
PACKAGE_URL = https://ez25coincore.org/
...
ZMQ_CFLAGS = 
ZMQ_LIBS = -lzmq
abs_builddir = /tmp/EZ25COIN
abs_srcdir = /tmp/EZ25COIN
abs_top_builddir = /tmp/EZ25COIN
abs_top_srcdir = /tmp/EZ25COIN
ac_ct_AR = ar
ac_ct_CC = gcc
ac_ct_CXX = g++
ac_ct_DUMPBIN = 
...
install_sh = ${SHELL} /tmp/EZ25COIN/build-aux/install-sh
...
pkgconfig_DATA = libez25coinconsensus.pc
EZ25COIND_BIN = $(top_builddir)/src/$(EZ25COIN_DAEMON_NAME)$(EXEEXT)
EZ25COIN_QT_BIN = $(top_builddir)/src/qt/$(EZ25COIN_GUI_NAME)$(EXEEXT)
EZ25COIN_CLI_BIN = $(top_builddir)/src/$(EZ25COIN_CLI_NAME)$(EXEEXT)
EZ25COIN_WIN_INSTALLER = $(PACKAGE)-$(PACKAGE_VERSION)-win$(WINDOWS_BITS)-setup$(EXEEXT)
empty := 
space := $(empty) $(empty)
OSX_APP = EZ25COIN-Qt.app
...
OSX_INSTALLER_ICONS = $(top_srcdir)/src/qt/res/icons/ez25coin.icns
OSX_PLIST = $(top_builddir)/share/qt/Info.plist #not installed
OSX_QT_TRANSLATIONS = da,de,es,hu,ru,uk,zh_CN,zh_TW
DIST_DOCS = $(wildcard doc/*.md) $(wildcard doc/release-notes/*.md)
DIST_CONTRIB = $(top_srcdir)/contrib/ez25coin-cli.bash-completion \
	       $(top_srcdir)/contrib/ez25coin-tx.bash-completion \
	       $(top_srcdir)/contrib/ez25coind.bash-completion \
...
WINDOWS_PACKAGING = $(top_srcdir)/share/pixmaps/ez25coin.ico \
...
COVERAGE_INFO = baseline.info \
  test_ez25coin_filtered.info total_coverage.info \
  baseline_filtered.info functional_test.info functional_test_filtered.info \
  test_ez25coin_coverage.info test_ez25coin.info

OSX_APP_BUILT = $(OSX_APP)/Contents/PkgInfo $(OSX_APP)/Contents/Resources/empty.lproj \
  $(OSX_APP)/Contents/Resources/ez25coin.icns $(OSX_APP)/Contents/Info.plist \
  $(OSX_APP)/Contents/MacOS/EZ25COIN-Qt $(OSX_APP)/Contents/Resources/Base.lproj/InfoPlist.strings
...
EXTRA_DIST = $(DIST_SHARE) test/functional/test_runner.py \
	...
	test/util/ez25coin-util-test.py \
	test/util/data/ez25coin-util-test.json \
...
CLEANFILES = $(OSX_DMG) $(EZ25COIN_WIN_INSTALLER)
...
src/config/ez25coin-config.h: src/config/stamp-h1
	@test -f $@ || rm -f src/config/stamp-h1
	@test -f $@ || $(MAKE) $(AM_MAKEFLAGS) src/config/stamp-h1

src/config/stamp-h1: $(top_srcdir)/src/config/ez25coin-config.h.in $(top_builddir)/config.status
	@rm -f src/config/stamp-h1
	cd $(top_builddir) && $(SHELL) ./config.status src/config/ez25coin-config.h
$(top_srcdir)/src/config/ez25coin-config.h.in:  $(am__configure_deps) 
	($(am__cd) $(top_srcdir) && $(AUTOHEADER))
	rm -f src/config/stamp-h1
	touch $@

distclean-hdr:
	-rm -f src/config/ez25coin-config.h src/config/stamp-h1
libez25coinconsensus.pc: $(top_builddir)/config.status $(srcdir)/libez25coinconsensus.pc.in
...
$(EZ25COIN_WIN_INSTALLER): all-recursive
	$(MKDIR_P) $(top_builddir)/release
	STRIPPROG="$(STRIP)" $(INSTALL_STRIP_PROGRAM) $(EZ25COIND_BIN) $(top_builddir)/release
	STRIPPROG="$(STRIP)" $(INSTALL_STRIP_PROGRAM) $(EZ25COIN_QT_BIN) $(top_builddir)/release
	STRIPPROG="$(STRIP)" $(INSTALL_STRIP_PROGRAM) $(EZ25COIN_CLI_BIN) $(top_builddir)/release
...
$(OSX_APP)/Contents/Resources/ez25coin.icns: $(OSX_INSTALLER_ICONS)
	$(MKDIR_P) $(@D)
	$(INSTALL_DATA) $< $@

$(OSX_APP)/Contents/MacOS/EZ25COIN-Qt: all-recursive
	$(MKDIR_P) $(@D)
	STRIPPROG="$(STRIP)" $(INSTALL_STRIP_PROGRAM)  $(EZ25COIN_QT_BIN) $@
...
$(APP_DIST_EXTRAS): $(APP_DIST_DIR)/$(OSX_APP)/Contents/MacOS/EZ25COIN-Qt
...
$(APP_DIST_DIR)/$(OSX_APP)/Contents/MacOS/EZ25COIN-Qt: $(OSX_APP_BUILT) $(OSX_PACKAGING)
...
#deploy: $(OSX_DMG)
#deploy: $(EZ25COIN_WIN_INSTALLER)

$(EZ25COIN_QT_BIN): FORCE
	$(MAKE) -C src qt/$(@F)

$(EZ25COIND_BIN): FORCE
	$(MAKE) -C src $(@F)

$(EZ25COIN_CLI_BIN): FORCE
	$(MAKE) -C src $(@F)
...
#test_ez25coin.info: baseline_filtered.info
#	$(MAKE) -C src/ check
#	$(LCOV) -c $(LCOV_OPTS) -d $(abs_builddir)/src -t test_ez25coin -o $@
#	$(LCOV) -z $(LCOV_OPTS) -d $(abs_builddir)/src

#test_ez25coin_filtered.info: test_ez25coin.info
#	$(abs_builddir)/contrib/filter-lcov.py $(LCOV_FILTER_PATTERN) $< $@
#	$(LCOV) -a $@ $(LCOV_OPTS) -o $@

#functional_test.info: test_ez25coin_filtered.info
...
#test_ez25coin_coverage.info: baseline_filtered.info test_ez25coin_filtered.info
#	$(LCOV) -a $(LCOV_OPTS) baseline_filtered.info -a test_ez25coin_filtered.info -o $@

#total_coverage.info: test_ez25coin_filtered.info functional_test_filtered.info
#	$(LCOV) -a $(LCOV_OPTS) baseline_filtered.info -a test_ez25coin_filtered.info -a functional_test_filtered.info -o $@ | $(GREP) "\%" | $(AWK) '{ print substr($$3,2,50) "/" $$5 }' > coverage_percent.txt

#test_ez25coin.coverage/.dirstamp:  test_ez25coin_coverage.info
...
#cov: test_ez25coin.coverage/.dirstamp total.coverage/.dirstamp
...
clean-local: clean-docs
	rm -rf coverage_percent.txt test_ez25coin.coverage/ total.coverage/ test/tmp/ cache/ $(OSX_APP)
